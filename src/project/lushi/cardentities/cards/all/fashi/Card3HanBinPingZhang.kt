...
import project.lushi.findimageentities.findcardimage.FindHanBinPingZhang

class Card3HanBinPingZhang(imageFind: BaseImageFind = FindHanBinPingZhang(), cardLocationMode: CardLocationMode = CardLocationMode.IN牌库, baseAtkNum: Int = 0, baseBloodNum: Int = 0, attribute: Attribute = Attribute.法术, pay: Int = 3, is嘲讽: Boolean = false, is磁力: Boolean = false, is吸血: Boolean = false, is冲锋: Boolean = false, is突袭: Boolean = false, is双生: Boolean = false, is发现: Boolean = false, is随从: Boolean = false, is奥秘: Boolean = true)
